...
def next_state_tx(aw1, aw2, w1, w2, maxCountNow, maxCountNext, countNow, countNext, ownerNow, ownerNext):
    return And(w2 == w1,
...
def constructor(xa1, xn1, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, maxCountNow, maxCountNext, t_maxCount, countNow, countNext, t_count, ownerNow, ownerNext, t_owner):
    return If(Not(xn1==0), next_state_tx(awNow, awNext, wNow, wNext, maxCountNow, maxCountNext, countNow, countNext, ownerNow, ownerNext), 
	And(And(
	t_owner[0] == xa1,
	And(
	t_maxCount[0] == 1,
	t_count[0] == 0)), next_state_tx(awNow, awNext, wNow, wNext, t_maxCount[0], maxCountNext, t_count[0], countNext, t_owner[0], ownerNext)))

def pay(xa1, xn1, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, maxCountNow, maxCountNext, t_maxCount, countNow, countNext, t_count, ownerNow, ownerNext, t_owner):
    return If(Not(xn1==0), next_state_tx(awNow, awNext, wNow, wNext, maxCountNow, maxCountNext, countNow, countNext, ownerNow, ownerNext), 
	And(If(
	Not(countNow<maxCountNow), 
		next_state_tx(awNow, awNext, wNow, wNext, maxCountNow, maxCountNext, countNow, countNext, ownerNow, ownerNext), And(
		And(If(And(xa1==ownerNow),And(t_count[0] == countNow+1, True, wNow==t_w[0], awNow==t_aw[0]),And(And(
	t_count[0] == countNow,
	If(
	Not(wNow >= 0), 
		next_state_tx(awNow, awNext, wNow, wNext, maxCountNow, maxCountNext, countNow, countNext, ownerNow, ownerNext), And(send(xa1, wNow, wNow, t_w[0], awNow, t_aw[0]), True))))),next_state_tx(t_aw[0], awNext, t_w[0], wNext, maxCountNow, maxCountNext, t_count[0], countNext, ownerNow, ownerNext))))))
...
def step_trans(f1, xa1, xn1,  aw1, aw2, w1, w2, t_aw, t_w, block_num1, block_num2, i, maxCountNow, maxCountNext, t_maxCount, countNow, countNext, t_count, ownerNow, ownerNext, t_owner):
    return And(And(xa1 >= 0, xa1 <= A, xn1 >= 0),
               And([aw1[j] >= 0 for j in range(A+1)]),
               block_num2 >= block_num1,
               	pay(xa1, xn1, aw1, aw2, w1, w2, t_aw, t_w, block_num1, maxCountNow, maxCountNext, t_maxCount, countNow, countNext, t_count, ownerNow, ownerNext, t_owner))

new_state = And(And(xa[0] >= 0, xa[0] <= A, xn[0] >= 0),
               And([aw[0][j] >= 0 for j in range(A+1)]),
                  constructor(xa[0], xn[0],  aw[0], aw[1], w[0], w[1], t_aw[0], t_w[0], block_num[0], maxCount[0], maxCount[1], t_maxCount[0], count[0], count[1], t_count[0], owner[0], owner[1], t_owner[0]))
s.add(new_state)
for i in range(1, N):
    new_state = step_trans(f[i], xa[i], xn[i],  aw[i],
                           aw[i+1], w[i], w[i+1], t_aw[i], t_w[i], block_num[i], block_num[i+1], i, maxCount[i], maxCount[i+1], t_maxCount[i], count[i], count[i+1], t_count[i], owner[i], owner[i+1], t_owner[i])
    s.add(new_state)
...
#                       ForAll([xn_q, f_q, w_q, *aw_q, *t_w_q, *t_awq_list, maxCount_q, *t_maxCount_q, count_q, *t_count_q, owner_q, *t_owner_q ], Or(Not(step_trans(f_q, xa_q, xn_q, aw[i], aw_q, w[i], w_q, t_aw_q, t_w_q, i, maxCount[i], maxCount_q, t_maxCount_q, count[i], count_q, t_count_q, owner[i], owner_q, t_owner_q)), w_q > 0)))))
