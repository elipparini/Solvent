...
def next_state_tx(aw1, aw2, w1, w2, ownerNow, ownerNext):
    return And(w2 == w1,
...
def constructor(xa1, xn1, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, ownerNow, ownerNext, t_owner):
    return If(Not(xn1==0), next_state_tx(awNow, awNext, wNow, wNext, ownerNow, ownerNext), 
	And(t_owner[0] == xa1, next_state_tx(awNow, awNext, wNow, wNext, t_owner[0], ownerNext)))

def pay(xa1, xn1, pay_amount, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, ownerNow, ownerNext, t_owner):
    return If(Not(xn1==0), next_state_tx(awNow, awNext, wNow, wNext, ownerNow, ownerNext), 
	And(If(
	Not(pay_amount<=wNow), 
		next_state_tx(awNow, awNext, wNow, wNext, ownerNow, ownerNext), And(
		And(If(And(ownerNow==xa1),And(If(
	Not(pay_amount >= 0), 
		next_state_tx(awNow, awNext, wNow, wNext, ownerNow, ownerNext), And(send(xa1, pay_amount, wNow, t_w[0], awNow, t_aw[0]), True))),And(next_state_tx(awNow, awNext, wNow, wNext, ownerNow, ownerNext), True, t_w[0]==wNow, t_aw[0]==awNow)),next_state_tx(t_aw[0], awNext, t_w[0], wNext, ownerNow, ownerNext))))))
...
def step_trans(f1, xa1, xn1, pay_amount, aw1, aw2, w1, w2, t_aw, t_w, block_num1, block_num2, i, ownerNow, ownerNext, t_owner):
    return And(And(xa1 >= 0, xa1 <= A, xn1 >= 0),
               And([aw1[j] >= 0 for j in range(A+1)]),
               block_num2 >= block_num1,
               	pay(xa1, xn1, pay_amount, aw1, aw2, w1, w2, t_aw, t_w, block_num1, ownerNow, ownerNext, t_owner))

new_state = And(And(xa[0] >= 0, xa[0] <= A, xn[0] >= 0),
               And([aw[0][j] >= 0 for j in range(A+1)]),
                  constructor(xa[0], xn[0],  aw[0], aw[1], w[0], w[1], t_aw[0], t_w[0], block_num[0], owner[0], owner[1], t_owner[0]))
s.add(new_state)
for i in range(1, N):
    new_state = step_trans(f[i], xa[i], xn[i], pay_amount[i], aw[i],
                           aw[i+1], w[i], w[i+1], t_aw[i], t_w[i], block_num[i], block_num[i+1], i, owner[i], owner[i+1], t_owner[i])
    s.add(new_state)
...
#                       ForAll([xn_q, f_q, w_q, *aw_q, *t_w_q, *t_awq_list, pay_amount_q, owner_q, *t_owner_q ], Or(Not(step_trans(f_q, xa_q, xn_q, pay_amount_q, aw[i], aw_q, w[i], w_q, t_aw_q, t_w_q, i, owner[i], owner_q, t_owner_q)), w_q > 0)))))
#                       #ForAll([xn_q, f_q, w_q, *aw_q ], Or(Not(step_trans(f_q, xa_q, xn_q, aw[i], aw_q, w[i], w_q, t_aw[i], t_w[i], i)), w_q > 0)))))


def p_liquidity15a_nonliq_1(i):
    t_awq_list0 = [t_awq_m_j for t_awq_m in t_aw_q0 for t_awq_m_j in t_awq_m]; 
    return And(
        Exists([xa_q], And(user_is_legit(xa_q), True,
            ForAll([xn_q0, f_q0, w_q0, *aw_q0, *t_w_q0, *t_awq_list0, block_num_q0, pay_amount_q0, owner_q0, *t_owner_q0],  
                Or(
Not(step_trans(f_q0, xa_q, xn_q0, pay_amount_q0, aw[i], aw_q0, w[i], w_q0, t_aw_q0, t_w_q0, block_num[i], block_num_q0, i+0, owner[i], owner_q0, t_owner_q0)),

And([Or(j != xa_q, Not(aw_q0[j] == aw[i][j]+w[i])) for j in range(A+1)])
        )))))

queries = {}
queries['liquidity15a_nonliq'] = [[p_liquidity15a_nonliq_1(i)] for i in range(1, N)]


timeStart = time.time()
for prop in {'liquidity15a_nonliq'}:
    print()
    print('Property [' + prop + ']')
