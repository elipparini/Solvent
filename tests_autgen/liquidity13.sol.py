...
def pay(xa1, xn1, pay_amount, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, bNow, bNext, t_b):
    return If(Not(xn1==0), next_state_tx(awNow, awNext, wNow, wNext, bNow, bNext), 
	And(If(
	Not(And(pay_amount<=wNow,Not(bNow))), 
		next_state_tx(awNow, awNext, wNow, wNext, bNow, bNext), And(
		And(If(
	Not(pay_amount >= 0), 
		next_state_tx(awNow, awNext, wNow, wNext, bNow, bNext), And(send(xa1, pay_amount, wNow, t_w[0], awNow, t_aw[0]), next_state_tx(t_aw[0], awNext, t_w[0], wNext, bNow, bNext))),next_state_tx(t_aw[0], awNext, t_w[0], wNext, bNow, bNext))))))

def lock(xa1, xn1, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, bNow, bNext, t_b):
    return If(Not(xn1==0), next_state_tx(awNow, awNext, wNow, wNext, bNow, bNext), 
	And(If(
	Not(xa1==0), 
		next_state_tx(awNow, awNext, wNow, wNext, bNow, bNext), And(
...
def step_trans(f1, xa1, xn1, pay_amount, aw1, aw2, w1, w2, t_aw, t_w, block_num1, block_num2, i, bNow, bNext, t_b):
	...
	pay(xa1, xn1, pay_amount, aw1, aw2, w1, w2, t_aw, t_w, block_num1, bNow, bNext, t_b),
		lock(xa1, xn1, aw1, aw2, w1, w2, t_aw, t_w, block_num1, bNow, bNext, t_b)))
...
def p_liquidity13b_liq_1(i):
    t_awq_list0 = [t_awq_m_j for t_awq_m in t_aw_q0 for t_awq_m_j in t_awq_m]; 
    return And(
        Exists([xa_q], And(user_is_legit(xa_q), b[i]==False,
            ForAll([xn_q0, f_q0, w_q0, *aw_q0, *t_w_q0, *t_awq_list0, block_num_q0, pay_amount_q0, b_q0, *t_b_q0],  
                Or(
Not(step_trans(f_q0, xa_q, xn_q0, pay_amount_q0, aw[i], aw_q0, w[i], w_q0, t_aw_q0, t_w_q0, block_num[i], block_num_q0, i+0, b[i], b_q0, t_b_q0)),
...
queries = {}
queries['liquidity13b_liq'] = [[p_liquidity13b_liq_1(i)] for i in range(1, N)]
queries['liquidity13a_nonliq'] = [[p_liquidity13a_nonliq_1(i)] for i in range(1, N)]


timeStart = time.time()
for prop in {'liquidity13b_liq','liquidity13a_nonliq'}:
    print()
    print('Property [' + prop + ']')
