...
constructor_p_q0 = Int("constructor_p0_q")
...
def constructor(xa1, xn1, constructor_p, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, p1Now, p1Next, t_p1, p2Now, p2Next, t_p2):
    return And(t_w[0] == wNow + xn1, 
	And(And(
	t_p1[0] == xa1,
	t_p2[0] == constructor_p), next_state_tx(awNow, awNext, t_w[0], wNext, t_p1[0], p1Next, t_p2[0], p2Next)))

def deposit(xa1, xn1, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, p1Now, p1Next, t_p1, p2Now, p2Next, t_p2):
    return And(t_w[0] == wNow + xn1, 
	And(If(
...
def pay(xa1, xn1, awNow, awNext, wNow, wNext, t_aw, t_w, block_num, p1Now, p1Next, t_p1, p2Now, p2Next, t_p2):
	...
	Not(xa1==p1Now), 
		...
		And(If(
	Not(wNow >= 0), 
		next_state_tx(awNow, awNext, wNow, wNext, p1Now, p1Next, p2Now, p2Next), And(send(xa1, wNow, wNow, t_w[0], awNow, t_aw[0]), next_state_tx(t_aw[0], awNext, t_w[0], wNext, p1Now, p1Next, p2Now, p2Next))),next_state_tx(t_aw[0], awNext, t_w[0], wNext, p1Now, p1Next, p2Now, p2Next))))))
...
def step_trans(f1, xa1, xn1,  aw1, aw2, w1, w2, t_aw, t_w, block_num1, block_num2, i, p1Now, p1Next, t_p1, p2Now, p2Next, t_p2):
    return And(And(xa1 >= 0, xa1 <= A, xn1 >= 0),
               And([aw1[j] >= 0 for j in range(A+1)]),
               block_num2 >= block_num1,
               If(f1 == Proc.deposit,
	deposit(xa1, xn1, aw1, aw2, w1, w2, t_aw, t_w, block_num1, p1Now, p1Next, t_p1, p2Now, p2Next, t_p2),
		pay(xa1, xn1, aw1, aw2, w1, w2, t_aw, t_w, block_num1, p1Now, p1Next, t_p1, p2Now, p2Next, t_p2)))

new_state = And(And(xa[0] >= 0, xa[0] <= A, xn[0] >= 0),
               And([aw[0][j] >= 0 for j in range(A+1)]),
                  constructor(xa[0], xn[0], constructor_p[0],  aw[0], aw[1], w[0], w[1], t_aw[0], t_w[0], block_num[0], p1[0], p1[1], t_p1[0], p2[0], p2[1], t_p2[0]))
s.add(new_state)
for i in range(1, N):
    new_state = step_trans(f[i], xa[i], xn[i],  aw[i],
                           aw[i+1], w[i], w[i+1], t_aw[i], t_w[i], block_num[i], block_num[i+1], i, p1[i], p1[i+1], t_p1[i], p2[i], p2[i+1], t_p2[i])
    s.add(new_state)
