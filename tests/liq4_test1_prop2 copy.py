...
def pay(xa1, xn1, pay_amount, awNow, awNext, wNow, wNext, t_aw, t_w, block_num):
    return If(
                ...
                And(If(
                    Not(pay_amount<=wNow-1), 
                    next_state_tx(awNow, awNext, wNow, wNext), 
                    And(And(If(
                        Not(pay_amount >= 0), 
                        next_state_tx(awNow, awNext, wNow, wNext), 
                        And(send(xa1, pay_amount, wNow, t_w[0], awNow, t_aw[0]), next_state_tx(t_aw[0], awNext, t_w[0], wNext))),next_state_tx(t_aw[0], awNext, t_w[0], wNext))))))
"""
    ...
    return If(
                ...
                And(If(
                    ...
                    And(And(If(
                        Not(pay_amount >= 0), 
                        next_state_tx(awNow, awNext, wNow, wNext), 
                        And(send(xa1, pay_amount, wNow, t_w[0], awNow, t_aw[0]), next_state_tx(t_aw[0], awNext, t_w[0], wNext))))))))
...
def p1(i):
    ...
    return And(
        Exists([xa_q], And(user_is_legit(xa_q), w[i]>1,
...
And([Or(j != xa_q, Not(aw_q0[j] == aw[i][j]+w[i]-1)) for j in range(A+1)])
        )))))
